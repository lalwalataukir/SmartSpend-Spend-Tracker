...
import {
  View, Text, ScrollView, TouchableOpacity, StyleSheet, SafeAreaView, Alert, TextInput, Modal, Share, Platform,
} from 'react-native';
...
import { formatDateForCSV, formatTimeForCSV, formatCurrency } from '../../src/utils/format';
...
export default function SettingsScreen() {
  ...
  const handleExportCSV = async () => {
    try {
      ...
      await Share.share({
        message: csv,
        title: `SpendSmart_Export_${new Date().toISOString().slice(0, 7)}.csv`,
      });
    } catch (error) {
  ...
  const SettingsItem = ({ icon, label, onPress, rightContent, destructive }: {
    icon: keyof typeof Ionicons.glyphMap; label: string; onPress?: () => void;
    rightContent?: React.ReactNode; destructive?: boolean;
  }) => (
    <TouchableOpacity
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface }]}>
            ...
            {themeModes.map(t => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  themeMode === t.key && { backgroundColor: colors.primary + '15' },
                ]}
              >
          ...
          <View style={styles.themeRow}>
            {PAYMENT_METHODS.map(method => (
              <TouchableOpacity
                key={method}
                testID={`default-payment-${method}`}
                onPress={() => setDefaultPaymentMethod(method)}
                style={[
                  styles.themeChip,
                  { borderColor: defaultPaymentMethod === method ? colors.primary : colors.border },
                  defaultPaymentMethod === method && { backgroundColor: colors.primary },
                ]}
              >
                <Text style={[styles.themeText, {
                  color: defaultPaymentMethod === method ? '#FFF' : colors.text,
                  fontWeight: defaultPaymentMethod === method ? '700' : '400',
                }]}>{method}</Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        <View style={[styles.settingsGroup, { backgroundColor: colors.surface }]}>
          <SettingsItem
            ...
            label="App Version"
            rightContent={<Text style={[styles.versionText, { color: colors.textSecondary }]}>1.0.0</Text>}
          />
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modal, { backgroundColor: colors.background }]}>
            ...
            <Text style={[styles.modalDesc, { color: colors.textSecondary }]}>
              This action cannot be undone. All transactions, budgets, and custom categories will be permanently deleted.
            </Text>
            <Text style={[styles.modalDesc, { color: colors.text, fontWeight: '600' }]}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                ...
                style={[styles.modalBtn, { backgroundColor: deleteConfirmText === 'DELETE' ? colors.danger : colors.border }]}
              >
...
const styles = StyleSheet.create({
  ...
  modalInput: {
    width: '100%', height: 48, borderRadius: Radius.md, borderWidth: 1,
