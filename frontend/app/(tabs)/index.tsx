...
export default function HomeScreen() {
  ...
  const loadData = useCallback(() => {
    ...
    setTodayTotal(getTotalForRange(db, today.start, today.end));
    setMonthTotal(getTotalForRange(db, month.start, month.end));
    setRecentTransactions(getRecentTransactions(db, 10));

    const categories = getAllCategories(db);
    const monthKey = getMonthYearKey(new Date());
    const budgets = getAllBudgetsForMonth(db, monthKey);
    const health: Array<{ category: Category; spent: number; limit: number }> = [];
    for (const budget of budgets) {
      const cat = categories.find(c => c.id === budget.categoryId);
      if (cat) {
        const spent = getTotalForCategoryInRange(db, cat.id, month.start, month.end);
...
const styles = StyleSheet.create({
  ...
  budgetChip: {
    width: 120, padding: Spacing.md, borderRadius: Radius.md,
    marginRight: Spacing.sm, borderWidth: 0.5,
  },
