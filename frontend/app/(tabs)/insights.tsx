...
import {
  View, Text, ScrollView, StyleSheet, SafeAreaView, TouchableOpacity, Dimensions,
} from 'react-native';
...
import {
  getTotalForRange, getCategorySpendingForRange, getDailySpendingForRange,
} from '../../src/db/database';
import { formatCurrency, formatMonthYear, getCurrentMonthRange, getLastMonthRange, getMonthYearKey } from '../../src/utils/format';
...
export default function InsightsScreen() {
  ...
  const loadData = useCallback(() => {
    ...
    const thisTotal = getTotalForRange(db, monthStart, monthEnd);
    const prevTotal = getTotalForRange(db, prevMonthStart, prevMonthEnd);
    setMonthTotal(thisTotal);
    setLastMonthTotal(prevTotal);

    const daysInMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).getDate();
    const today = new Date();
    const daysPassed = currentDate.getMonth() === today.getMonth() && currentDate.getFullYear() === today.getFullYear()
      ? today.getDate() : daysInMonth;
    setDailyAvg(daysPassed > 0 ? thisTotal / daysPassed : 0);

    setCategorySpending(getCategorySpendingForRange(db, monthStart, monthEnd));
    setDailySpending(getDailySpendingForRange(db, monthStart, monthEnd));
  }, [db, currentDate]);
  ...
  if (monthDiff > 10) {
    nudges.push(`📈 You're spending ${Math.abs(Math.round(monthDiff))}% more than last month.`);
  } else if (monthDiff < -10) {
    nudges.push(`📉 Great! You're spending ${Math.abs(Math.round(monthDiff))}% less than last month.`);
  }
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]} testID="insights-screen">
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.monthSelector}>
          <TouchableOpacity testID="prev-month-btn" onPress={() => setCurrentDate(subMonths(currentDate, 1))}>
            <Ionicons name="chevron-back" size={24} color={colors.primary} />
          </TouchableOpacity>
          <Text style={[styles.monthText, { color: colors.text }]}>{formatMonthYear(currentDate)}</Text>
          <TouchableOpacity testID="next-month-btn" onPress={() => setCurrentDate(addMonths(currentDate, 1))}>
          ...
          <View style={styles.summaryRow}>
            <View style={styles.summaryMini}>
              <Ionicons
                ...
                size={16}
                color={monthDiff >= 0 ? colors.danger : colors.success}
              />
              <Text style={[styles.diffText, { color: monthDiff >= 0 ? colors.danger : colors.success }]}>
                {monthDiff >= 0 ? '+' : ''}{Math.round(monthDiff)}% vs last month
              </Text>
            </View>
            <View style={styles.summaryMini}>
              ...
              return (
                <View key={cat.categoryId} style={styles.barRow}>
                  <View style={styles.barLabel}>
                    <Text style={styles.barEmoji}>{cat.categoryEmoji}</Text>
                    <Text style={[styles.barName, { color: colors.text }]} numberOfLines={1}>{cat.categoryName}</Text>
                  </View>
                  ...
                  <View key={d.day} style={styles.dailyBarWrapper}>
                    <View style={[styles.dailyBarTrack, { backgroundColor: colors.border }]}>
                      <View style={[styles.dailyBarFill, {
                        height: `${(d.total / maxDailySpend) * 100}%`,
