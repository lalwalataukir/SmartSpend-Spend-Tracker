...
export default function HistoryScreen() {
  ...
  const [snackbar, setSnackbar] = useState<{ visible: boolean; message: string; undoData?: any }>({ visible: false, message: '' });

  const loadData = useCallback(() => {
    setCategories(getAllCategories());
    let txs: TransactionWithCategory[];
    if (searchQuery.trim()) {
      txs = searchTransactions(searchQuery.trim());
    } else if (activeFilter === 'this_month') {
      const range = getCurrentMonthRange();
      txs = getTransactionsByDateRange(range.start, range.end);
    } else if (activeFilter === 'last_month') {
      const range = getLastMonthRange();
      txs = getTransactionsByDateRange(range.start, range.end);
    } else if (activeFilter === 'category' && selectedCategoryId) {
      txs = getAllTransactions().filter(t => t.categoryId === selectedCategoryId);
    } else {
      txs = getAllTransactions();
    }
    setTransactions(txs);
  }, [searchQuery, activeFilter, selectedCategoryId]);
  ...
  const handleUndo = () => {
    if (snackbar.undoData) {
      ...
      insertTransaction({ amount: b.amount, categoryId: b.categoryId, note: b.note, date: b.date, paymentMethod: b.paymentMethod, isRecurring: b.isRecurring, recurringIntervalDays: b.recurringIntervalDays, isSplit: b.isSplit, splitShare: b.splitShare });
      loadData();
    }
  ...
  const filters: { key: FilterType; label: string }[] = [
    { key: 'all', label: 'All' }, { key: 'this_month', label: 'This Month' },
    { key: 'last_month', label: 'Last Month' }, { key: 'category', label: 'Category' },
  ];
  ...
  return (
    <View style={[styles.safeArea, { backgroundColor: colors.background, paddingTop: insets.top }]} testID="history-screen">
      ...
      <View style={[styles.searchBar, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        <TextInput testID="search-input" placeholder="Search notes..." placeholderTextColor={colors.textSecondary} value={searchQuery} onChangeText={setSearchQuery} style={[styles.searchInput, { color: colors.text }]} returnKeyType="search" />
