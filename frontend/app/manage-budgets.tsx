...
import {
  View, Text, ScrollView, TouchableOpacity, TextInput, StyleSheet, SafeAreaView, Modal,
} from 'react-native';
...
import {
  getAllCategories, getAllBudgetsForMonth, upsertBudget, deleteBudget,
  getTotalForCategoryInRange, type Category, type Budget,
...
export default function ManageBudgetsScreen() {
  ...
  const loadData = useCallback(() => {
    ...
    setCategories(cats);
    setBudgets(getAllBudgetsForMonth(db, monthKey));
    const spendMap = new Map<number, number>();
    for (const cat of cats) {
      spendMap.set(cat.id, getTotalForCategoryInRange(db, cat.id, monthRange.start, monthRange.end));
    }
  ...
  const handleSaveBudget = () => {
    ...
    upsertBudget(db, selectedCategory.id, amount, monthKey);
    setShowModal(false);
    loadData();
  };
          ...
          return (
            <View key={cat.id} style={[styles.catRow, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                    ...
                    <View style={[styles.progressBar, { backgroundColor: colors.border }]}>
                      <View
                        style={[styles.progressFill, {
                          ...
                          backgroundColor: getBudgetColor(pct),
                        }]}
                      />
                    </View>
                    <Text style={[styles.budgetText, { color: colors.textSecondary }]}>
                      {formatCurrency(spent)} of {formatCurrency(budget.limitAmount)} ({pct}%)
                    </Text>
                  </>
                ) : (
              ...
              <View style={styles.actions}>
                <TouchableOpacity
                  testID={`set-budget-${cat.id}`}
                  onPress={() => openSetBudget(cat)}
                  style={[styles.actionBtn, { backgroundColor: colors.primary + '10' }]}
                >
                  <Ionicons name={budget ? 'pencil' : 'add'} size={18} color={colors.primary} />
                </TouchableOpacity>
                {budget && (
                  <TouchableOpacity
                    testID={`remove-budget-${cat.id}`}
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modal, { backgroundColor: colors.background }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>
              {selectedCategory ? `Budget for ${selectedCategory.emoji} ${selectedCategory.name}` : 'Set Budget'}
            </Text>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                ...
                disabled={!budgetAmount.trim() || parseFloat(budgetAmount) <= 0}
                style={[styles.modalBtn, {
                  backgroundColor: budgetAmount.trim() && parseFloat(budgetAmount) > 0 ? colors.primary : colors.border,
                }]}
              >
