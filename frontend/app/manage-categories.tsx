...
import {
  View, Text, ScrollView, TouchableOpacity, TextInput, StyleSheet, SafeAreaView, Modal, Alert,
} from 'react-native';
...
export default function ManageCategoriesScreen() {
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    if (editingCategory) {
      updateCategory(db, { ...editingCategory, name: name.trim(), emoji, colorHex });
    } else {
      insertCategory(db, { name: name.trim(), emoji, isDefault: 0, colorHex });
    }
    setShowModal(false);
    loadData();
  };
  ...
  const handleDelete = (cat: Category) => {
    if (cat.isDefault) {
      Alert.alert('Cannot Delete', 'Default categories cannot be deleted. You can rename them.');
      return;
    }
    Alert.alert('Delete Category', `Are you sure you want to delete "${cat.name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => { deleteCategory(db, cat.id); loadData(); } },
    ]);
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]} testID="manage-categories-screen">
      <View style={styles.header}>
        ...
        <TouchableOpacity testID="add-category-btn" onPress={openAdd} style={[styles.addBtn, { backgroundColor: colors.primary }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        {categories.map(cat => (
          <View key={cat.id} style={[styles.catRow, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={[styles.catIcon, { backgroundColor: cat.colorHex + '20' }]}>
              <Text style={styles.catEmoji}>{cat.emoji}</Text>
            </View>
            <View style={styles.catInfo}>
              <Text style={[styles.catName, { color: colors.text }]}>{cat.name}</Text>
              {cat.isDefault ? (
                <Text style={[styles.catBadge, { color: colors.textSecondary }]}>Default</Text>
              ) : null}
            </View>
            <TouchableOpacity testID={`edit-cat-${cat.id}`} onPress={() => openEdit(cat)} style={styles.actionBtn}>
              <Ionicons name="pencil" size={18} color={colors.primary} />
            </TouchableOpacity>
            {!cat.isDefault && (
              <TouchableOpacity testID={`delete-cat-${cat.id}`} onPress={() => handleDelete(cat)} style={styles.actionBtn}>
          ...
          <View style={[styles.modal, { backgroundColor: colors.background }]}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.emojiRow}>
              {COMMON_EMOJIS.map(e => (
                <TouchableOpacity
                  key={e}
                  onPress={() => setEmoji(e)}
                  style={[styles.emojiBtn, emoji === e && { backgroundColor: colors.primary + '20', borderColor: colors.primary }]}
                >
            ...
            <View style={styles.colorRow}>
              {PRESET_COLORS.map(c => (
                <TouchableOpacity
                  ...
                  style={[styles.colorBtn, { backgroundColor: c }, colorHex === c && styles.colorBtnSelected]}
                />
              ))}
            </View>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                ...
                onPress={handleSave}
                disabled={!name.trim()}
                style={[styles.modalBtn, { backgroundColor: name.trim() ? colors.primary : colors.border }]}
              >
...
const styles = StyleSheet.create({
  ...
  catRow: {
    flexDirection: 'row', alignItems: 'center',
    padding: Spacing.md, borderRadius: Radius.md, marginBottom: Spacing.sm, borderWidth: 0.5,
  },
  catIcon: {
  ...
  input: {
    ...
    paddingHorizontal: Spacing.lg, fontSize: FontSize.base,
  },
  ...
  emojiBtn: {
    ...
    alignItems: 'center', justifyContent: 'center',
    marginRight: Spacing.sm, borderWidth: 1.5, borderColor: 'transparent',
  },
